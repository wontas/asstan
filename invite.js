...
client.on("guildMemberAdd", (member) => {
    ...
    guild.fetchInvites().then(invites => {
        ...
        if(channel){
       channel.send(`> <:welc:939912715336568862><:come:939912637800677447> **${member} Adlı Kişi Sunucuya Katıldı. Davet Eden Kişi ${invite.inviter.tag} Toplam ${total + bonus} Davete Ulaştı. Tebrikler!** `)
        }
    }).catch();
});
    ...
    db.add(`invites.${data.inviter}.leave`, 1);
     if(channel){
        let user = client.users.cache.get(data.inviter)
     	channel.send(`> <a:PepeExit:940289013187751947> **${member.user.tag} Sunucudan Ayrıldı. Davet Eden Kişi ${user.tag} Toplam Daveti ${Number(total) + Number(bonus)}** `)
     }
});
